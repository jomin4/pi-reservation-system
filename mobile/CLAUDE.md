...
**SSE 와 같은 판단이다.** 밖으로 나갔다 돌아오는 것을 인터페이스 뒤에 두고, 지금은 **가짜 결제창**(`Alert` 로 승인·취소를 묻는다)이 그 자리에 있다.

| | |
|---|---|
| 어댑터 | `src/payment/checkout.ts` — **결제창이 주는 건 `paymentKey` 하나**다 |
| 왜 진짜를 못 붙이나 | **상점 시크릿 키 · 백엔드 · dev build** 셋 다 없다. ⚠️ **Expo Go 는 커스텀 네이티브 모듈을 못 싣는다** — 토스 RN SDK 가 안 뜬다 |
| 가짜가 흉내 못 내는 것 | **앱이 백그라운드로 가는 구간.** `Alert` 는 앱 안에 머문다 — 복귀 처리(`E-04`)를 이걸로 검증했다고 적지 말 것 |

### ⚠️ 답이 없을 때 `POST` 를 다시 부르지 않는다

`202` 든 타임아웃이든 **「모른다」는 같은 상태**다. `awaitPaymentResult` 가 **`GET /holds/{id}/payment`** 로 묻는다 (`api.md` §5.3).

> **복귀 순서가 정해져 있다** — ① 선점 유효 ② 결제 승인 여부인데 **②를 먼저** 본다. 뒤집으면 **승인된 건을 만료로 오판**한다.
...
| ⚠️ **`replace` 를 `beforeRemove` 가드로 막기** | **확정 직후 예약 완료로 못 넘어간다.** `replace` 도 화면을 스택에서 뺀다 — 우리가 보낸 이동은 통과시킬 것 (#44) |
