 ...
 *
 * ⚠️ **`front/src/api/problem.ts` 와 같은 내용이다** (PR #85). 플랫폼에 안 묶인 코드라
 *    RN 에서도 그대로 돈다. `front`/`mobile` 은 workspace 로 안 묶여 있어(`tech.md`)
 *    공유 패키지가 없고, 그래서 **복사가 유일한 선택**이다.
 *
 *    **계약이 바뀌면 두 벌을 같이 고친다.** 어느 쪽이든 `schema.d.ts` 를 재생성하면
 *    타입이 안 맞아 `tsc` 가 잡는다 — 조용히 어긋나지는 않는다.
 */
