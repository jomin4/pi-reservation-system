...
> **이 문서가 답하는 질문 하나** — **어떻게 일하나.** 트랙 4개가 Claude 세션 4개로 병렬 개발하면서 서로를 깨뜨리지 않는 규칙.

| | |
|---|---|
| 확정일 | 2026-09-10 |
| 실체 | 부록의 설정 원본은 세팅 시점에 복사해 쓴다. 그때 조정될 수 있다 |
| 여기 있다 | worktree · 브랜치 · **계약 PR 선행** · 커밋 · 이슈 · PR · 머지 · **금지사항 검사** · 리뷰 |
| 여기 없다 → | CI 파이프라인·릴리스 → [deploy.md](deploy.md) · 금지사항의 근거 → [adr/](adr/) |
...
| | 결과 |
|---|---|
| 위반 7종 (1~7) | **7/7 검출** · `exit 1` |
| 위반 3종 (8~10) | ⏳ **픽스처 미작성** — `back/` 모듈이 생기면 같이 만든다 |
...
| [deploy.md](deploy.md) | CI 파이프라인 · 릴리스 절차 |

---

**← 앞** [deploy.md](deploy.md) — 어떻게 올리나 · **다음 →** [research.md](research.md) — 착수 전 조사
