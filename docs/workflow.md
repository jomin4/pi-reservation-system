...
| **트랙 도메인 문서** | ⚠️ **그 트랙 세션이 직접 고친다** — 아래 |
| **크로스트랙 문서** | **본체에서** `docs/*` 브랜치로 |
| `develop` 따라잡기 | 작업 중 수시로 `git merge origin/develop` |

### ⚠️ 1.3.1 문서를 누가 고치나 — 「내 트랙만 참조하는 절인가」

**처음 문구는 「문서 변경은 본체에서」였다. 그게 의도보다 넓게 읽혔다.**

> 모바일 세션이 `tech.md`의 모바일 버전을 **실측해놓고** 이 문구 때문에 손을 떼고 본체에 넘기려 했다. **실측한 쪽이 못 쓰고, 안 해본 쪽이 받아쓰게 된다.**

**가르는 기준은 파일이 아니라 절이다.**

| | **트랙 도메인 절** | **크로스트랙 문서** |
|---|---|---|
| 판별 | **내 트랙만 참조하나** | 두 트랙 이상이 참조한다 |
| 예 | `tech.md` 모바일 버전 표 · `api.md` §7의 트랙별 Mock 스위치 · `deploy.md` §8(mobile CI/CD) | `workflow.md` · `overview.md` · `features.md` · `data.md` · `api.md` §5(계약) |
| 누가 | **그 트랙 세션** | **본체** |
| 브랜치 | `docs/<track>-<slug>` | `docs/<slug>` |

**ADR은 파일이 아니라 결정의 범위를 따른다.**

| 결정이 | 누가 쓰나 | 예 |
|---|---|---|
| **한 트랙에 갇힌다** | **그 트랙** | [ADR-0008](../adr/0008-tailwind-major-split.md) — 모바일 Tailwind 메이저. **실측하고 저울질한 쪽이 쓴다** |
| 두 트랙 이상을 바꾼다 | **본체** | ADR-0001(모듈 경계) · ADR-0007(계약 관리) |

> **원래 막으려던 것은 그대로 남는다** — **두 세션이 같은 절을 동시에 고치는 것.** 그건 worktree도 못 막는다. 그런데 **모바일 표를 모바일 세션이 고치는 건 그 위험이 아니다.** 그 절을 만질 세션은 하나뿐이다.

⚠️ **애매하면 본체다.** 그리고 **트랙 도메인 문서라도 구현 PR과 섞지 않는다** — 문서 PR을 **먼저** 내고 그 위에 구현을 얹는다 (§3과 같은 순서).
