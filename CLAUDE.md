...
| **내 트랙만 참조하는 절** | ⚠️ **그 트랙이 직접 고친다** — `docs/<track>-<slug>` 브랜치 |
| **두 트랙 이상이 참조하는 문서** | **본체 worktree에서** `docs/*` 브랜치로 |

> **문서를 누가 고치나는 파일이 아니라 절로 가른다** (`workflow.md` §1.3.1). `tech.md`의 모바일 버전 표는 모바일이, `workflow.md`·`features.md`·`api.md` §5는 본체가. **ADR은 결정이 한 트랙에 갇히면 그 트랙이 쓴다.** 애매하면 본체다.
...
| **작업 끝** | 카드가 Done 이 되면 `Auto-close issue` 가 이슈를 닫는다. 안 닫히면 `gh issue close <번호>` |
